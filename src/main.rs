...
/// 
/// If 's' has the proper form, return 'Some<x,y>'. If it does not parse correctly,
/// return 'None'.
...
fn pixel_to_point(bounds: (usize, usize), pixel: (usize, usize), upper_left: Complex<f64>, lower_right: Complex<f64>) -> Complex<f64> {
    ...
    Complex {
        ...
        // Subtraction because pixel.1 increases as we go down,
        // but imaginary component increase as we go up.
...
///
/// If 'c' seems to be a member (more precisely, if we reached the iteration limit without being able to prove that 'x' is not a member),
/// return 'None'.
